...
构建一个本地可运行的 GitHub 仓库分析可视化平台。用户输入公开 GitHub 仓库 URL 后，系统通过后端代理聚合 GitHub API 数据，在 Vue 3 前端展示基础指标、语言分布、贡献者排行、活跃趋势、项目健康度，并结合本地 qwen3.6-plus 模型或规则降级方案生成 0-100 AI 项目评分和中文评语。
...
- [x] P1: AI 项目评分，优先调用本地 qwen3.6-plus，失败时使用确定性规则评分降级
- [x] P1: 响应式布局和基础视觉打磨
...
- AI 评分模块封装为独立服务：先尝试 OpenAI-compatible 本地 LLM 接口，模型名默认 `qwen3.6-plus`；如未配置或调用失败，返回规则评分和说明。
